...
export default function StaffReports() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div><h1 className="text-2xl font-bold text-slate-800 dark:text-white">Reports</h1><p className="text-slate-500 dark:text-slate-400">Generate and download system reports</p></div>
      ...
      <div className="card">
        ...
        <div className="space-y-4">
          ...
          <div><label className="label">Download Format</label><div className="flex gap-3"><button onClick={() => handleExport('csv')} className="btn-outline"><FileSpreadsheet className="w-4 h-4" /> Export as CSV</button><button onClick={() => handleExport('json')} className="btn-outline"><FileText className="w-4 h-4" /> Export as JSON</button></div></div>
        </div>
      </div>
    </div>
  );
