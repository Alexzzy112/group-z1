    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">{['all', 'low', 'moderate', 'high', 'critical'].map(c => (<button key={c} onClick={() => setFilterCat(c)} className={`btn-sm ${filterCat === c ? 'btn-primary' : 'btn-secondary'}`}>{c}</button>))}</div>
        </div>
